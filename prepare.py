...
def fraction(decoder='s'):
    '''
    N = 536 students
    M = 19 events
    actions: N x M
    lengths of sequences: N
    exercises: N x M (the (t + 1)th)
    targets: N x M, binary
    metadata: nb_events x nb_features, sparse
    indices: N x M, ID of current event in [|0, nb_events - 1|]
    '''
    REPEAT = 1
    # Load data
    with open('/home/jj/code/qna/data/fraction.json') as f:
        answers = np.array(json.load(f)['student_data'], dtype=np.int32)
    nb_students, nb_questions = answers.shape
    with open('/home/jj/code/qna/data/qmatrix-fraction.json') as f:
    ...
    for line in q:
        ...
        skills.add(code + '0')
        skills.add(code + '1')
    encode = dict(zip(skills, range(1, 10000)))  # Codes should start from 1
    ...
    for _ in range(REPEAT):
        for i in range(nb_students):
            ...
            exercises[i].extend(np.arange(1, nb_questions))
            lengths[i] += nb_questions - 1
            targets[i].extend(answers[i][1:])
    ...
    # if decoder == 's':  # FIXME should be i
...
def truncate(a, b, c):
    ...
    b_ = b.reshape(-1, MAX)
    c_ = c.reshape(-1, MAX)
    kept = ~np.all(a_ == 0, axis=1)
    return a_[kept], b_[kept], c_[kept]
...
def assistments():
    ...
    if os.path.isfile('data/assistments09/data.npz'):
        ...
        # df = pd.read_csv('/Users/jin/code/seq/data/assistments09/data.csv')
        # y = np.load('/Users/jin/code/ktm/data/assistments09/y-swf.npy')
    ...
    # df = pd.read_csv('/Users/jin/code/seq/data/dummy/data.csv')
    dt = time.time()
    df = pd.read_csv('/home/jj/code/ktm/data/assistments09/data.csv')
    ...
    print('Per user')
    values = list(nb_samples_per_user.values())
    print('min', min(values))
    print('avg', np.mean(values))
    print('std', np.std(values))
    print('max', max(values))
    
    max_length = 1100 #max(nb_samples_per_user.values()) - 1
    ...
    act, exe, tar = truncate(actions, exercises, targets)
    print('act exe tar', act.shape, exe.shape, tar.shape)
    #sys.exit(0)
    
    dt = time.time()
    np.savez('data/assistments09/data.npz', actions=act, lengths=lengths, exercises=exe, targets=tar)
    print('saved', time.time() - dt)
    # sys.exit(0)
    return act, lengths, exe, tar
...
#         exercises = snapshot['exercises']
#         targets = snapshot['targets']
#         # df = pd.read_csv('/Users/jin/code/seq/data/assistments2/data.csv')
#         # y = np.load('/Users/jin/code/ktm/data/assistments2/y-swf.npy')
#         # print(targets.shape, y.shape)
#         # print(targets[:10])
#         # print(y[:10])
#         # assert all(np.array(df['correct']) == y)
